...
const outputPath = path.resolve(__dirname, '../questions_export_fixed.json');


// Match \\ (2 backslashes) in memory (which is \\\\ in literal regex)
...
function fixText(text: string): string {
    if (!text) return text;
    // Replace 4 backshashes with 2, if followed by command-like char
    ...
    const fixedQuestions = questions.map((q: any) => {
        ...
        if (q.options && Array.isArray(q.options)) {
            const newOptions = q.options.map((opt: any) => {
                ...
                // If option is object with "value" or "text" or "latex"
            ...
            q.options = newOptions; // Assign potentially new array
        }

        // Fix micro_explanations (JSON object)
        if (q.micro_explanations && typeof q.micro_explanations === 'object') {
            // It's a map of key -> string
