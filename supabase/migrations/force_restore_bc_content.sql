-- ============================================================
-- FORCE RESTORE BC CONTENT (Delete & Re-Insert)
-- This script ensures all BC-only content is present.
-- ============================================================

-- 1. Explicitly DELETE potential partial BC data to ensure clean state
DELETE FROM public.sections WHERE id IN (
    '6.11', '6.12', '6.13', -- Unit 6 BC
    '7.5', '7.9',           -- Unit 7 BC
    '8.13'                  -- Unit 8 BC
) AND course_scope = 'bc_only';

-- Delete Unit 9 and 10 sections entirely to re-populate
DELETE FROM public.sections WHERE topic_id IN ('BC_Unit9', 'BC_Series');
-- Delete Unit 9 and 10 topics to re-populate
DELETE FROM public.topics WHERE id IN ('BC_Unit9', 'BC_Series');


-- 2. INSERT TOPICS (Unit 9 & 10)
...
-- 3. INSERT SECTIONS (All BC Only Content)
...
-- UNIT 9: Parametric/Polar (Full Unit)
INSERT INTO public.sections (topic_id, id, title, description, course_scope, estimated_minutes, sort_order, is_unit_test) VALUES
