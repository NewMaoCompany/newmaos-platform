-- Repopulate Unit 6 Skills and Error Tags
-- Generated per User Request
-- Standardizes "Unit6" to "ABBC_Integration"

BEGIN;

-- 1. Ensure schema supports 'unit' column on error_tags (Safety check)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'error_tags' AND column_name = 'unit') THEN
        ALTER TABLE public.error_tags ADD COLUMN unit VARCHAR(50);
    END IF;
END $$;

-- 2. Delete existing Unit 6 entries (using both possible identifiers to be safe)
DELETE FROM public.skills 
WHERE unit IN ('Unit6', 'ABBC_Integration');

DELETE FROM public.error_tags 
WHERE unit IN ('Unit6', 'ABBC_Integration');
