-- FIX: Analysis Badge 24 Hour Refresh Logic
-- Allows the Analysis badge to appear if there is ANY activity in the past 24 hours relative to the client's date,
-- ensuring it appears even if the user viewed the analysis before midnight.
...
BEGIN
    ...
    -- 4. Analysis Badge: has new question attempts in the past 24 hours
    ...
    ELSE
        SELECT EXISTS (
            ...
            AND created_at >= (p_client_date - INTERVAL '1 day')
        ) INTO v_analysis_badge;
    END IF;

    -- 5. Forum Badge: unread notifications + unread activities
