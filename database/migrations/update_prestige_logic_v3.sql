-- Update Inject Stardust RPC (V3: Robust Progression & Cost Handling)
-- This version handles star_level = 3 as a valid state for final promotion
...
BEGIN
    ...
    -- V2/V3 Cost logic: 100 * power(5, planet - 1)
    ...
    ELSIF v_current_star >= 2 THEN
        -- Handles both level 2 and potential stuck level 3
    ...
    ELSE
        ...
        IF v_current_planet < 10 THEN
            ...
            WHERE user_id = v_user_id;
        ELSE
            -- Max planet reached, cap at star 3
            UPDATE user_prestige
            SET 
                current_stardust = current_stardust - v_req_stardust,
                star_level = 3,
