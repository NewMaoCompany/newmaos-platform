-- ============================================================
-- Migration: Fix Check-in Notification Logic v7
-- Description: Update generate_system_notifications to remove generic welcome,
--              add practice reminders, and keep daily check-in logic.
-- ============================================================
...
DECLARE
    ...
    v_has_checked_in_today BOOLEAN;
...
BEGIN
    ...
    -- Check if user has CHECKED IN TODAY (Manual check-in)
    v_has_checked_in_today := EXISTS (
    ...
    -- REMOVED: 0. WELCOME NOTIFICATION (Notification Only)
    -- auth.ts now strictly handles the initial "gift_claim" welcome message.
    -- We no longer generate duplicate generic welcome messages.

    -- ============================================================
    -- PRACTICE NOTIFICATION
    -- ============================================================
    v_prefix := '🎯 Ready to practice?';
    IF NOT v_has_practiced_today THEN
        IF NOT EXISTS (
            ...
            VALUES (p_user_id, '🎯 Ready to practice? Try algorithm push, random questions, or review your mistakes!', '/practice', true);
        END IF;
    ELSE
        -- Clear if practiced
    ...
    IF v_has_practiced_today AND v_solved IS NOT NULL AND v_solved > 0 THEN
        ...
        IF NOT EXISTS (
            SELECT 1 FROM public.notifications 
            WHERE user_id = p_user_id AND text LIKE v_prefix || '%' AND created_at::DATE = CURRENT_DATE
        ) THEN
    ...
    IF NOT v_has_practiced_today AND NOT v_has_checked_in_today THEN
        SELECT COUNT(*) INTO v_unread_count FROM public.notifications 
        WHERE user_id = p_user_id AND (text LIKE v_prefix || '%' OR text LIKE '[Daily Check-in]%') AND unread = true;
        IF v_unread_count = 0 THEN
            IF NOT EXISTS (
                SELECT 1 FROM public.notifications 
                WHERE user_id = p_user_id AND (text LIKE v_prefix || '%' OR text LIKE '[Daily Check-in]%') AND created_at::DATE = CURRENT_DATE
            ) THEN
                INSERT INTO public.notifications (user_id, text, link, unread)
        ...
        END IF;
    ELSE
        -- Clear if practiced or checked-in
...
WHERE text = 'Welcome to NewMaoS! Start your first session.' 
AND type IS NULL;
