...
interface AppContextType {
    ...
    saveSectionProgress: (sectionId: string, data: any, timeSpent: number, entityType?: 'course' | 'unit' | 'section') => Promise<boolean>;
...
const fetchRadarData = async () => {
    ...
    // Definition of Units for Radar Axis (AB only for now as requested)
    const UNIT_MAPPING: { id: string; label: string }[] = [
        { id: 'ABBC_Limits', label: 'Limits' },
        { id: 'ABBC_Derivatives', label: 'Derivatives' },
        { id: 'ABBC_Composite', label: 'Composite' },
        { id: 'ABBC_Applications', label: 'Contextual Applic' },
        { id: 'ABBC_Analytical', label: 'Analytical Applications' },
        { id: 'ABBC_Integration', label: 'Integration' },
        { id: 'ABBC_DiffEq', label: 'Diff Eq' },
        { id: 'ABBC_AppIntegration', label: 'App of Int' }
    ];
    ...
    try {
        ...
        if (data) {
            data.forEach((row: any) => {
                // "When unit total score reaches 85, it counts as full marks (100%)"
                // Assume row.score is percentage (0-100). If null, use correct/total ratio if valid.
                let rawScore = row.score;
                if (rawScore === null && row.total_questions > 0) {
                    rawScore = (row.correct_questions / row.total_questions) * 100;
                }
                rawScore = Number(rawScore) || 0;

                // Logic: Mastery = min(raw_score / 85, 1.0) * 100
                scoresMap[row.unit_id] = Math.min(rawScore / 85, 1.0) * 100;
            });
        }
...
const saveSectionProgress = async (sectionId: string, data: any, timeSpent: number, entityType: 'course' | 'unit' | 'section' = 'section'): Promise<boolean> => {
    try {
        ...
        setSectionProgressMap(prev => {
            ...
            // Don't overwrite 'completed' status with 'in_progress' (e.g. from auto-save)
            ...
            return {
                ...
                [sectionId]: {
                    ...
                    status: 'in_progress', // Saving implies in_progress
                    data: data,
                    // Type-safe updates
...
const completeSectionSession = async (sectionId: string, score: number, totalQuestions: number, correctQuestions: number, data: any): Promise<boolean> => {
    try {
        ...
        if (entityType === 'unit' || entityType === 'section') {
            // Refresh radar data if we updated a unit (or a section rolled up to unit)
            fetchRadarData();
        }
...
const fetchAllUserProgress = async () => {
    try {
        // Use RPC to bypass potential RLS issues with direct select
        // NOTE: RPC uses auth.uid(), no params needed
        ...
        console.log('fetchAllUserProgress raw data:', data); // DEBUG
        ...
        if (data) {
            ...
            console.log('Progress cache updated:', Object.keys(map).length, 'entries');
        }
    } catch (error) {
        console.error('fetchAllUserProgress exception:', error);
    }
};
...
useEffect(() => {
    if (user.id && isAuthenticated) {
        fetchAllUserProgress(); // No args needed, uses auth.uid() in RPC
    } else {
